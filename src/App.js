...
export default function App() {
  const [query, setQuery] = useState("EVIL DEAD");
    ...
    <>
      ...
      <Main>
        <Box>
          {isLoading && <Loader />}
          {!isLoading && !error && (
            <MovieList movies={movies} onSetMovie={handleSelectMovie} />
          )}
...
function MovieList({ movies, onSetMovie }) {
  return (
    <ul className="list list-movies">
      {movies?.map((movie) => (
        <Movie movie={movie} key={movie.imdbID} onSetMovie={onSetMovie} />
      ))}
    </ul>
  );
}
function Movie({ movie, onSetMovie }) {
  return (
    <li onClick={() => onSetMovie(movie.imdbID)}>
